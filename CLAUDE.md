...
이 프로젝트에는 `wwise_error_kb.json`에 239개의 공식 Wwise 에러 문서(한국어)가 포함되어 있습니다.
에러 코드가 KB에 매칭되면 프롬프트에 "공식 Wwise 문서" 섹션으로 자동 포함됩니다.
공식 문서의 원인/해결 단계를 기반으로 분석하되, 해당 오브젝트의 구체적 상황에 맞게 보충하세요.
